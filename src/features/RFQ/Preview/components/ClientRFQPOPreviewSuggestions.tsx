...
export function ClientRFQPOPreviewSuggestions({
  isMobile,
  ...
  return (
              ...
              >
                {Object.keys(structuredRfqComments ?? {}).length > 0 ? (
                  <Box className="my-1" sx={{ width: "100%" }}>
                    <Box className="pb-2">
                      <span className={`fw-600 ${isMobile ? "fs-8" : "fs-7"}`}>
                        {pdf ? "Comments" : t("common.comments")}
                      </span>
                    </Box>

                    {Object.entries(structuredRfqComments ?? {}).map(
                      ([key, value], i) => (
                        <CardContent
                          key={i}
                          sx={{
