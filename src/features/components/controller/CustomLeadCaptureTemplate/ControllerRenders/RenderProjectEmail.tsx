...
const RenderProjectEmail = ({ disabled = false }: { disabled?: boolean }) => {
  ...
  return (
    <div>
      ...
      >
          ...
          helperText={
            !!leadCaptureData?.leadEmail && emailError ? (
              <p className="error-text my-0">
                {t("errors.pleaseVerifyTheEmailAddress", {
                  defaultValue: "Please verify the Email address",
                })}
              </p>
            ) : (
              ""
        ...
        {leadCaptureData?.leadEmail === "" && (
          <FormHelperText sx={{ color: "error.main" }}>
            {t("common.pleaseEnterTheEmail", {
              defaultValue: "Please Enter the Email",
            })}
          </FormHelperText>
        )}
        {emailtimer !== null && (
          <p className="text-left my-0 error-text">
            {`${t(
              "twoFactorAuth.pleaseEnterVerificationCodeSentToYourInbox"
            )} ${formatDuration(emailtimer)}`}{" "}
          </p>
        )}
      </Grid>
      {emailtimer !== null && (
            ...
            InputProps={{
              endAdornment: (
                <LoadingButton
                  ...
                  disabled={
                    disabled || emailOTP?.length !== 6 || isGenerateOtpLoading
                  }
                  sx={{
                    width: "90px",
                    height: "40px",
...
export default RenderProjectEmail;
