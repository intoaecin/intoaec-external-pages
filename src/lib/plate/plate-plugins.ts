...
import { SpacerBlockElement } from "@/components/plate-ui/spacer-block";
...
const createSpacerPlugin = createMyPluginFactory({
  component: SpacerBlockElement,
  key: ELEMENT_SPACER,
  isElement: true,
});
export const plugins = createPlugins(
        ...
        onPaste: (editor) => (event) => {
          ...
          >((resolve, reject) => {
            ...
            for (let i = 0; i < (files?.length || 0); i++) {
              ...
              if (file) {
                ...
                reader.onload = (e) => {
                  ...
                  if (i === files.length - 1) {
                    resolve(fileList); // Resolve the promise after the last file is processed
                  }
                };
              }
            }
            ...
            value?.map(async (file) => {
              const res = await helperUpload(file);
  ...
  ],
  {
    components: withPlaceholders({
      ...
      [ELEMENT_SPACER]: SpacerBlockElement,
